...
(function ($, undefined) {
			...
			if (events) {
				...
				if (url) {
					...
					$.ajax($.extend({}, ajaxDefaults, source, {
						...
						error: function error(jqxhr, status, errorFn) {
  ...
  ------------------------------------------------------------------------------*/
		...
		function getSourcePrimitive(source) {
			return ((typeof source === 'undefined' ? 'undefined' : _typeof(source)) == 'object' ? source.events || source.url : '') || source;
		}
	}
  ...
  ---------------------------------------------------------------------------------*/
	...
	function DayEventRenderer() {
		...
		function buildHTMLForSegment(segment, index) {
			...
			html += " class='" + classNames.join(' ') + "'" + " data-date='" + htmlEscape(formatDate(event.start, 'yyyy-MM-dd')) + "'" + " data-rate='" + htmlEscape(event.rate || '') + "'" + " data-room-type='" + htmlEscape(event.roomType || '') + "'";
			if (segment.event.toolTipData && segment.event.toolTipData.bestAvailableRate !== "" && event.toolTipData !== 'undefined') {
				html += "qtipfc qtip-template='/assets/partials/reservation/rvTooltipContent.html'" + "qtip-template-object='segments[" + index + "]' qtip-at='" + tooltipPosition.at + "' qtip-my='" + tooltipPosition.my + "'";
			}

			html += " style=" + "'" + "position:absolute;" + "left:" + segment.left + "px;" + skinCss + "'" + ">" + "<div class='fc-event-inner'>";
			if (!event.allDay && segment.isStart) {
				html += "<span class='fc-event-time'>" + htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) + "</span>";
			}
			html += "<span class='fc-event-day'>" + htmlEscape(event.day || '') + "</span>";
			...
			html += htmlEscape(event.title || '') + "</span>" + "</div>";
